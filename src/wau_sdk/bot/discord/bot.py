"""bot.discord.bot — DiscordBot stub(Stage 0 脚手架)

对齐 wau-go-sdk/bot/discord/discord.go。

Stage 0:只定义 DiscordBot + chain 方法,无实际 Bot Gateway 连接。
...
class DiscordBot(Bot):
    ...
    async def start(self) -> None:
        ...
        Stage 1 实装:Discord Bot Gateway (WebSocket)。
        """
    ...
    def with_tenant(self, tenant_id: str) -> "DiscordBot":
        """设置 tenant_id,返回 Bot 支持链式调用"""
        self.tenant = tenant_id
        return self

    def with_universe(self, universe: str) -> "DiscordBot":
...
def new_discord_bot(token: str, builder: BotBuilder) -> DiscordBot:
    """用 token + builder 创建 Discord bot(stub)

    对齐 wau-go-sdk/bot/discord/discord.go:22 New。
    """
    return DiscordBot(token=token, builder=builder)
