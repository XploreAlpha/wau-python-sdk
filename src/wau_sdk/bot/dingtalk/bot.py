"""bot.dingtalk.bot — DingTalkBot stub (Stage 0 脚手架)

对齐 wau-go-sdk/bot/dingtalk/dingtalk.go (per W5 2026-07-13 closure)。
W6 (2026-07-09) W5 缺口补全:Python SDK 补 dingtalk 子包。

Stage 0:只定义 DingTalkBot + chain 方法,无实际 DingTalk SDK 调用。
Stage 1:实装 DingTalk Stream SDK (open-dingtalk/dingtalk-stream-sdk-python 或
        com.dingtalk:dingtalk-stream Java 移植版),Stage 1 路径在 W6.2 落地。
...
DingTalk Bot 平台说明:
  - 企业内部机器人:app_key + app_secret 鉴权(不同于群机器人)
  - 接收消息:Stream Callback (HTTP webhook / WebSocket)
  - 发送消息:OpenAPI /v1.0/im/bot/messages/send
"""
...
class DingTalkBot(Bot):
    ...
    对齐 wau-go-sdk/bot/dingtalk/dingtalk.go DingTalkBot 字段:
        ...
        app_secret: str  — DingTalk App Secret (robot code secret)
        tenant: str
        universe: str
    ...
    async def start(self) -> None:
        ...
        Stage 1 实装:DingTalk Stream SDK (dingtalk_stream.ChatBot + event dispatcher)
        或 HTTP webhook 模式 + OpenAPI client (DingTalkClient(access_token))。
        """
        # TODO(stage1): dingtalk_stream.ChatBot(app_key, app_secret) 注册回调
        return None
